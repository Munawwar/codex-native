...
export function setupNativeBinding(): void {
  ...
  // Check dist/ first (build output), then npm/<platform>/
  if (platform === "darwin") {
    const suffix = arch === "arm64" ? "arm64" : "x64";
    candidatePaths.push(`${rootDir}/dist/codex_native.darwin-${suffix}.node`);
    candidatePaths.push(`${rootDir}/npm/darwin-${suffix}/codex_native.darwin-${suffix}.node`);
  } else if (platform === "win32") {
    const suffix = arch === "arm64" ? "arm64" : "x64";
    candidatePaths.push(`${rootDir}/dist/codex_native.win32-${suffix}-msvc.node`);
    ...
    candidatePaths.push(`${rootDir}/dist/codex_native.linux-${arch}-${suffix}.node`);
