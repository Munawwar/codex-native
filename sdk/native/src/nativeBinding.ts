...
function resolveBindingEntryCandidates(): string[] {
  ...
  if (typeof __dirname === "string") {
    pushCandidate(path.resolve(__dirname, "..", "index.js"));
    pushCandidate(path.resolve(__dirname, "index.js"));
  }
  ...
  if (importMetaUrl) {
    try {
      ...
      pushCandidate(path.resolve(path.dirname(filePath), "..", "index.js"));
      pushCandidate(path.resolve(path.dirname(filePath), "index.js"));
    } catch {
  ...
  pushCandidate(path.resolve(process.cwd(), "index.js"));
  return candidates;
}
...
function resolvePlatformPackageName(): string | null {
  if (process.platform === "darwin") {
    if (process.arch === "arm64") {
      return "@codex-native/sdk-darwin-arm64";
    }
    if (process.arch === "x64") {
      return "@codex-native/sdk-darwin-x64";
    }
    return null;
  }
  ...
  if (process.platform === "linux") {
    ...
    if (process.arch === "arm64") {
      return `@codex-native/sdk-linux-arm64-${suffix}`;
    }
    if (process.arch === "x64") {
      return `@codex-native/sdk-linux-x64-${suffix}`;
    }
    return null;
  }
  ...
  if (process.platform === "win32") {
    if (process.arch === "arm64") {
      return "@codex-native/sdk-win32-arm64-msvc";
    }
    if (process.arch === "x64") {
      return "@codex-native/sdk-win32-x64-msvc";
    }
    return null;
  }
...
export function getNativeBinding(): NativeBinding | null {
  ...
  const bindingEntryCandidates = resolveBindingEntryCandidates();
  ...
  for (const candidate of bindingEntryCandidates) {
    if (!fs.existsSync(candidate)) {
      continue;
    }
  ...
  for (const root of packageRoots) {
    try {
      ...
      const nodeFile = entries.find((entry) => entry.endsWith(".node"));
      if (!nodeFile) {
        continue;
      }
      const binding = tryRequireNativeBinding(requireFn, path.join(root, nodeFile));
      if (binding) {
        cachedBinding = binding;
        return cachedBinding;
      }
  ...
  const error =
    lastError ??
    new Error(
      `Native binding entrypoint not found. Checked: ${bindingEntryCandidates.join(
        ", ",
      )} and ${packageRoots.join(", ")}`,
    );
